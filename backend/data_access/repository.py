...
class DataRepositoryBase(ABC):
    ...
    @abstractmethod
    def get_recent_telemetry(self, machine_id: str, minutes: int = 5) -> list[dict]: ...
...
class CsvDataRepository(DataRepositoryBase):
    ...
    def get_recent_telemetry(self, machine_id: str, minutes: int = 5) -> list[dict]:
        """Last `minutes` pings for a machine, oldest first - the rolling
        window the hazard-prediction model reads at request time. In this
        CSV-backed demo there's no live clock, so 'recent' means the most
        recent rows in the dataset for that machine."""
        df = self.telemetry[self.telemetry["machine_id"] == machine_id]
        if df.empty:
            return []
        df = df.sort_values("timestamp").tail(minutes)
        return df.to_dict(orient="records")
